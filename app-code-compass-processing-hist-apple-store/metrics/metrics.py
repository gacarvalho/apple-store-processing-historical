...
def validate_ingest(spark: SparkSession, df: DataFrame) -> tuple:
    ...
    if invalid_rating.count() > 0:
        ...
        validation_results["type_consistency_check"]["message"] = "Valores não numéricos encontrados na coluna 'im_rating'."
    else:
        validation_results["type_consistency_check"]["status"] = True
        validation_results["type_consistency_check"]["code"] = 300
        validation_results["type_consistency_check"]["message"] = "Consistência de tipo verificada com sucesso."
